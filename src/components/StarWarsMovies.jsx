...
const StarWarsMovies = () => {
  ...
  useEffect(() => {
    const fetchStarWarsMovies = async () => {
      try {
        setLoading(true);
        const response = await globalAPI.getStarWarsMovies();
        setMovies(response.data.results.slice(0, 30)); // Get up to 30 movies
        setError(null);
      } catch (err) {
        setError('Failed to fetch Star Wars movies');
        console.error('Error fetching Star Wars movies:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        <div className="text-white text-xl">Loading Star Wars movies...</div>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="mb-12">
        <h2 className="text-white text-2xl font-bold mb-6 ml-4">Star Wars Movies</h2>
