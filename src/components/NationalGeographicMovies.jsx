...
const NationalGeographicMovies = () => {
  ...
  useEffect(() => {
    const fetchNationalGeographicMovies = async () => {
      try {
        setLoading(true);
        const response = await globalAPI.getNationalGeographicMovies();
        setMovies(response.data.results.slice(0, 30)); // Get up to 30 movies
        setError(null);
      } catch (err) {
        setError('Failed to fetch National Geographic movies');
        console.error('Error fetching National Geographic movies:', err);
      } finally {
        setLoading(false);
      }
    };

    fetchNationalGeographicMovies();
  }, []);
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        <div className="text-white text-xl">Loading National Geographic movies...</div>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="mb-12">
        <h2 className="text-white text-2xl font-bold mb-6 ml-4">National Geographic Movies</h2>
...
export default NationalGeographicMovies; 