...
const NationalGeographicTVSeries = () => {
  ...
  useEffect(() => {
    const fetchNationalGeographicTVSeries = async () => {
      try {
        setLoading(true);
        const response = await globalAPI.getNationalGeographicTVSeries();
        setSeries(response.data.results.slice(0, 30)); // Get up to 30 series
        setError(null);
      } catch (err) {
        console.error('Error fetching National Geographic TV series:', err);
        setError('Failed to fetch National Geographic TV series');
      } finally {
        setLoading(false);
      }
    };

    fetchNationalGeographicTVSeries();
  }, []);
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        <div className="text-white text-xl">Loading National Geographic TV series...</div>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="mb-12">
        <h2 className="text-white text-2xl font-bold mb-6 ml-4">National Geographic TV Series</h2>
...
export default NationalGeographicTVSeries; 