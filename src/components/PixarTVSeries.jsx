...
const PixarTVSeries = () => {
  ...
  useEffect(() => {
    const fetchPixarTVSeries = async () => {
      try {
        setLoading(true);
        const response = await globalAPI.getPixarTVSeries();
        setSeries(response.data.results.slice(0, 30)); // Get up to 30 series
        setError(null);
      } catch (err) {
        console.error('Error fetching Pixar TV series:', err);
        setError('Failed to fetch Pixar TV series');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        <div className="text-white text-xl">Loading Pixar TV series...</div>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="mb-12">
        <h2 className="text-white text-2xl font-bold mb-6 ml-4">Pixar TV Series</h2>
