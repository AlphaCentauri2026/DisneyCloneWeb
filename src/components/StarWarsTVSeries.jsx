...
const StarWarsTVSeries = () => {
  ...
  useEffect(() => {
    const fetchStarWarsTVSeries = async () => {
      try {
        setLoading(true);
        const response = await globalAPI.getStarWarsTVSeries();
        setSeries(response.data.results.slice(0, 30)); // Get up to 30 series
        setError(null);
      } catch (err) {
        console.error('Error fetching Star Wars TV series:', err);
        setError('Failed to fetch Star Wars TV series');
      } finally {
        setLoading(false);
      }
    };

    fetchStarWarsTVSeries();
  }, []);
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        <div className="text-white text-xl">Loading Star Wars TV series...</div>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="mb-12">
        <h2 className="text-white text-2xl font-bold mb-6 ml-4">Star Wars TV Series</h2>
...
export default StarWarsTVSeries; 