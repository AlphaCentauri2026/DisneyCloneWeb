...
const GenreMovies = ({ genreId, genreName }) => {
  ...
  useEffect(() => {
    const fetchMoviesByGenre = async () => {
      try {
        ...
        setMovies(response.data.results.slice(0, 20)); // Get up to 20 movies per genre
        setError(null);
      } catch (err) {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        <div className="text-white text-xl">Loading {genreName} movies...</div>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="mb-12">
        <h2 className="text-white text-2xl font-bold mb-6 ml-4">{genreName}</h2>
